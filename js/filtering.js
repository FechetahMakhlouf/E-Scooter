...
function applyFilters() {
  ...
  var activeCategory = document.querySelector(
    ".filter-tag[data-category].active",
  );
  if (activeCategory) {
  ...
  if (products.length === 0) {
    var emptyTitle =
      typeof getTranslation === "function"
        ? getTranslation("models.empty.title") || "Aucun modèle trouvé"
        : "Aucun modèle trouvé";
    var emptyDesc =
      typeof getTranslation === "function"
        ? getTranslation("models.empty.desc") ||
          "Essayez d'autres critères de recherche"
        : "Essayez d'autres critères de recherche";
    grid.innerHTML =
      '<div style="grid-column:1/-1;text-align:center;padding:4rem 0;"><p style="font-size:3rem;margin-bottom:1rem;">&#128269;</p><h3>' +
      emptyTitle +
      '</h3><p style="color:var(--text-secondary);">' +
      emptyDesc +
      "</p></div>";
    return;
  }
  ...
  for (var i = 0; i < products.length; i++) {
    ...
    if (product.colors && product.colors.length > 0) {
      ...
      for (var c = 0; c < maxColors; c++) {
        colorDots +=
          '<div class="color-dot-card" style="background:' +
          product.colors[c].hex +
          ';"></div>';
      }
      colorDots += "</div>";
    }
    ...
    // Specs
    var vitesse =
      product.specs["Vitesse Max"] || product.specs["Vitesse"] || "-";
    var autonomie = product.specs["Autonomie"] || "-";
    var moteur = product.specs["Moteur"]
      ? product.specs["Moteur"].split(" ")[0]
      : "-";
    ...
    if (product.oldPrice) {
      oldPriceHtml =
        '<span style="text-decoration:line-through;color:var(--text-muted);margin-left:0.5rem;font-size:0.9rem;">' +
    ...
    var subtitleTxt =
      typeof getProductText === "function"
        ? getProductText(product, "subtitle", lang)
        : product.subtitle;
    var hl = searchQuery;
    var nameHl = highlightText(product.name, hl);
    var subtitleHl = highlightText(subtitleTxt || "", hl);
    var badgeKey =
      product.category === "electric-motorcycle" ? "badge.moto" : "badge.trot";
    var badgeLabel =
      typeof getTranslation === "function"
        ? getTranslation(badgeKey)
        : product.categoryLabel;
    var vitesseLabel =
      typeof getTranslation === "function"
        ? getTranslation("spec.Vitesse Max") || "Vitesse"
        : "Vitesse";
    var autonomieLabel =
      typeof getTranslation === "function"
        ? getTranslation("spec.Autonomie") || "Autonomie"
        : "Autonomie";
    var moteurLabel =
      typeof getTranslation === "function"
        ? getTranslation("spec.Moteur") || "Moteur"
        : "Moteur";
    var btnLabel =
      typeof getTranslation === "function"
        ? getTranslation("btn.view")
        : "Voir →";

    html +=
      '<div class="card product-card" style="animation-delay:' +
      delay +
      's">' +
      ...
      '<span style="font-size:1.25rem;font-weight:800;background:var(--gradient-primary);-webkit-background-clip:text;background-clip:text;color:transparent;">' +
